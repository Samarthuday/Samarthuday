...
ROWS = [
    ("host",),
    ("kv", "Role", "CS Graduate | Tech Intern @ Eli Lilly"),
    ("kv", "Direction", "Quantitative Research"),
    ("kv", "Focus", "Financial ML, Quant Finance, ML Systems"),
    ("kv", "Goal", "MS Financial Engineering | Fall 2027"),
    ("kv", "Location", "India"),
    ("gap",),

    ("sec", "Stack"),
    ("kv", "Languages", "Python, SQL"),
    ("kv", "Math", "Probability, Statistics, Linear Algebra"),
    ("kv", "Data", "NumPy, pandas, SciPy"),
    ("kv", "ML", "PyTorch, scikit-learn, XGBoost"),
    ("kv", "Finance", "Time Series, Risk Modelling, Backtesting"),
    ("kv", "Systems", "FastAPI, Docker, GitHub Actions"),
    ("gap",),

    ("sec", "Projects"),
    ("bul", "Financial risk and fraud modelling"),
    ("bul", "Market and time-series analysis"),
    ("bul", "Quantitative research projects"),
    ("bul", "Production ML systems"),
    ("gap",),

    ("sec", "Current"),
    ("bul", "Preparing for quant research and MSFE"),
    ("bul", "Building a financial ML portfolio"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">samarth@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">samarth</tspan><tspan fill="{MUTED}">@</tspan>'
